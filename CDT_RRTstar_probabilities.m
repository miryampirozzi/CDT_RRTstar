...
for loopp = 1: 1 
    ...
    for f=1:num_of_sequences
        ...
        for i = 1:numel(visited_order) - 1
            ...
            all_common_vertices=[all_common_vertices; common_vertices];
        ...
        %     x_circle = x_center + safety_margin * cos(theta);
        %     y_circle = y_center + safety_margin * sin(theta);
        ...
        for b=1:num_paths
            ...
            stop=false;
            probabilities = zeros(size(samples.points,1), 1);
            % for i = 1:num_samples:size(samples.points, 1)-num_samples+1
            %     probabilities(i)=0.30;
            %     probabilities(i+num_samples-1)=0.30;
            %     for j=i+1:i+num_samples-2
            %         middle_prob = 0.40 / (num_samples - 2);
            %         probabilities(j) = middle_prob;
            %     end
            % end
            ed=1;
            for i = 1:num_samples:size(samples.points, 1)-num_samples+1
                ...
                % disp(i);
                % disp('---------------------');
                dist1=sqrt((points_sequence(end,1) - triangulation.Points(all_common_vertices(ed,1),1))^2 + (points_sequence(end,2) - triangulation.Points(all_common_vertices(ed,1),2))^2);
                dist2=sqrt((points_sequence(end,1) - triangulation.Points(all_common_vertices(ed,2),1))^2 + (points_sequence(end,2) - triangulation.Points(all_common_vertices(ed,2),2))^2);
                
                minor=min(dist1, dist2);

                if minor==dist1 && norm(triangulation.Points(all_common_vertices(ed,1),:) - samples.points(i,:))<100
                    probabilities(i)=0.1;
                    probabilities(i+1)=0.7;
                    for j=i+2:i+num_samples-1
                        middle_prob = 0.2 / (num_samples - 2);
                        probabilities(j) = middle_prob;
                    end
                elseif minor==dist1 && norm(triangulation.Points(all_common_vertices(ed,1),:) - samples.points(i+num_samples-1,:))<100 % <20 for the first two maps and <100 for the third map
                    probabilities(i+num_samples-1)=0.1;
                    probabilities(i+num_samples-2)=0.7;
                    for j=i+num_samples-3:-1:i
                        middle_prob = 0.2 / (num_samples - 2);
                        probabilities(j) = middle_prob;
                    end
                elseif minor==dist2 && norm(triangulation.Points(all_common_vertices(ed,2),:) - samples.points(i,:))<100
                    probabilities(i)=0.1;
                    probabilities(i+1)=0.7;
                    for j=i+2:i+num_samples-1
                        middle_prob = 0.2 / (num_samples - 2);
                ...
                elseif minor==dist2 && norm(triangulation.Points(all_common_vertices(ed,2),:) - samples.points(i+num_samples-1,:)) <100
                    probabilities(i+num_samples-1)=0.1;
                    probabilities(i+num_samples-2)=0.7;
                    for j=i+num_samples-3:-1:i
                        middle_prob = 0.2 / (num_samples - 2);
                ...
                while contin
                    ...
                    % if probabilities(i+num_samples-1)>probabilities(i)
                    %     probabilities(i:i+num_samples-1,:)=flip(probabilities(i:i+num_samples-1,:));
                    %     random_index__ = randsample(1, probabilities(i:i+num_samples-1,:));
                    %     random_index_=num_samples-random_index__+1;
                    % else
                    %     random_index_ = randsample(1, probabilities(i:i+num_samples-1,:));
                    % end

                    random_index_ = randsample(1, probabilities(i:i+num_samples-1,:));
                    random_index=random_index_+i-1;
                    % disp('Random index:');
                    % disp(random_index);
